...
from .serializers import (
    OrganizationSerializer, OrganizationCreateSerializer, TeamSerializer, 
    TeamCreateSerializer, OrganizationMembershipSerializer, TeamMembershipSerializer,
    OrganizationInviteSerializer, TeamInviteSerializer, OrganizationStatsSerializer
)
from .permissions import (
    IsOrganizationMember, IsOrganizationAdmin, IsOrganizationOwner,
    IsTeamMember, IsTeamLeadOrOrganizationAdmin
)


@extend_schema_view(
    list=extend_schema(summary="List user's organizations", description="Get all organizations where the user is a member"),
    create=extend_schema(summary="Create organization", description="Create a new organization with the user as owner"),
    retrieve=extend_schema(summary="Get organization details", description="Get detailed information about an organization"),
    update=extend_schema(summary="Update organization", description="Update organization details (admin/owner only)"),
    partial_update=extend_schema(summary="Partially update organization", description="Partially update organization details (admin/owner only)"),
    destroy=extend_schema(summary="Delete organization", description="Delete an organization (owner only)"),
)
class OrganizationViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).distinct().select_related('created_by').prefetch_related('memberships__user')
...
@extend_schema_view(
    list=extend_schema(summary="List organization teams", description="Get all teams in the organization"),
    create=extend_schema(summary="Create team", description="Create a new team in the organization"),
    retrieve=extend_schema(summary="Get team details", description="Get detailed information about a team"),
    update=extend_schema(summary="Update team", description="Update team details (team lead or org admin only)"),
    partial_update=extend_schema(summary="Partially update team", description="Partially update team details (team lead or org admin only)"),
    destroy=extend_schema(summary="Delete team", description="Delete a team (team lead or org admin only)"),
)
class TeamViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Team.objects.filter(
            organization_id=organization_id,
            is_archived=False
        ).select_related('organization', 'created_by').prefetch_related('memberships__user')
    ...
    @extend_schema(
        ...
        description="Get all members of the team"
    )
    @action(detail=True, methods=['get'])
    ...
    @action(detail=True, methods=['post'], permission_classes=[IsTeamLeadOrOrganizationAdmin])
