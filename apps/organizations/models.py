...
class Organization(models.Model):
    ...
    subscription_plan = models.CharField(
        max_length=50, choices=SUBSCRIPTION_PLANS, default="free"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="created_organizations"
    )
...
class Team(models.Model):
    ...
    # Settings stored as JSON
    settings = models.JSONField(
        default=dict, blank=True, help_text="Team-specific settings and preferences"
    )

    # Special flags
    is_default = models.BooleanField(
        default=False, help_text="Default team for new organization members"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["organization", "is_default"], name="idx_team_is_default"
            ),
            models.Index(
                fields=["organization", "is_archived"], name="idx_team_is_archived"
            ),
...
class TeamMembership(models.Model):
    ...
    def clean(self):
        ...
        if not OrganizationMembership.objects.filter(
            organization=self.team.organization, user=self.user, status="active"
        ).exists():
